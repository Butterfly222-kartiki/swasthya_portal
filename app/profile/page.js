...
export default function ProfilePage() {
  ...
  return (
    <div className="animate-fade-in">
      {/* Profile header */}
      <div style={{ background: 'linear-gradient(135deg,#0f766e,#065f46)', borderRadius: 20, padding: '2rem', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 20 }}>
        <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#0d9488,#10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontFamily: 'DM Sans', fontWeight: 800, fontSize: '2rem', flexShrink: 0 }}>
          {profile.full_name?.[0]?.toUpperCase()}
        </div>
        <div>
          <h1 style={{ fontFamily: 'DM Sans', fontWeight: 800, fontSize: '1.5rem', color: 'white' }}>
          ...
          {profile.blood_group && (
            <span style={{ display: 'inline-block', marginTop: 8, padding: '3px 12px', borderRadius: 20, background: 'rgba(13,148,136,0.2)', border: '1px solid rgba(13,148,136,0.4)', color: '#5eead4', fontSize: '0.75rem', fontWeight: 700 }}>
              🩸 Blood Group: {profile.blood_group}
            </span>
          )}
        </div>
        <button onClick={() => setEditing(!editing)} style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.1)', color: 'white', cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: 600, fontSize: '0.85rem' }}>
          <Edit3 size={14} /> Edit
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, background: '#f5f5f5', borderRadius: 12, padding: 4, width: 'fit-content', marginBottom: 20 }}>
        {tabs.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', fontFamily: 'DM Sans', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', background: activeTab === tab ? 'white' : 'transparent', color: activeTab === tab ? '#0d9488' : '#3d6b66', boxShadow: activeTab === tab ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s', textTransform: 'capitalize' }}>
      ...
      {activeTab === 'profile' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 20 }}>
          <div className="card">
            <h3 style={{ fontFamily: 'DM Sans', fontWeight: 700, color: '#0f2d2a', marginBottom: 16 }}>Personal Information</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={field.key}>
                  <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#3d6b66', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>{field.label}</label>
                  {editing ? (
                    ...
                    <div style={{ color: '#0f2d2a', fontWeight: 500, fontSize: '0.9rem', padding: '8px 0', borderBottom: '1px solid #ccfbf1' }}>{profile[field.key] || '—'}</div>
                  )}
                </div>
              ))}

              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#3d6b66', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Gender</label>
                {editing ? (
                  <select className="input-field" value={profile.gender || ''} onChange={e => setProfile({ ...profile, gender: e.target.value })}>
                    <option value="">Select</option>
                    <option>Male</option><option>Female</option><option>Other</option>
                  </select>
                ) : <div style={{ color: '#0f2d2a', fontWeight: 500, fontSize: '0.9rem', padding: '8px 0', borderBottom: '1px solid #ccfbf1' }}>{profile.gender || '—'}</div>}
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#3d6b66', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Blood Group</label>
                {editing ? (
                  <select className="input-field" value={profile.blood_group || ''} onChange={e => setProfile({ ...profile, blood_group: e.target.value })}>
                    <option value="">Select</option>
                    {BLOOD_GROUPS.map(g => <option key={g}>{g}</option>)}
                  </select>
                ) : <div style={{ color: '#0f2d2a', fontWeight: 500, fontSize: '0.9rem', padding: '8px 0', borderBottom: '1px solid #ccfbf1' }}>{profile.blood_group || '—'}</div>}
              </div>
            </div>
            {editing && (
          ...
          <div className="card">
            <h3 style={{ fontFamily: 'DM Sans', fontWeight: 700, color: '#0f2d2a', marginBottom: 16 }}>Medical Information</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
        ...
        <div>
          ...
          {addingRx && (
            <div className="card" style={{ marginBottom: 16 }}>
              <h4 style={{ fontFamily: 'DM Sans', fontWeight: 700, color: '#0f2d2a', marginBottom: 14 }}>New Prescription</h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ...
                  <div key={f.key}>
                    <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#0f2d2a', marginBottom: 5 }}>{f.label}</label>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {prescriptions.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
                <Pill size={40} style={{ margin: '0 auto 12px', opacity: 0.2, color: '#0d9488' }} />
                <p style={{ color: '#3d6b66' }}>No prescriptions found</p>
              </div>
                ...
                <div className="prescription-header">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <Pill size={18} color="#5eead4" />
                      <span style={{ fontFamily: 'DM Sans', fontWeight: 700 }}>{rx.medicine}</span>
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
                    {[
                      ...
                      <div key={d.label}>
                        ...
                        <div style={{ fontWeight: 600, color: '#0f2d2a', marginTop: 2, fontSize: '0.875rem' }}>{d.value}</div>
                      </div>
                    ))}
                  </div>
                  {rx.notes && <div style={{ marginTop: 10, padding: '8px 12px', background: '#fafafa', borderRadius: 8, fontSize: '0.8rem', color: '#3d6b66' }}>📝 {rx.notes}</div>}
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === 'followups' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {followUps.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
              <RefreshCw size={40} style={{ margin: '0 auto 12px', opacity: 0.2, color: '#0d9488' }} />
              <p style={{ color: '#3d6b66' }}>No follow-ups scheduled</p>
            </div>
          ) : followUps.map(fu => (
            <div key={fu.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'DM Sans', fontWeight: 700, color: '#0f2d2a' }}>Follow-up with Dr. {fu.doctor?.full_name}</div>
                <div style={{ fontSize: '0.8rem', color: '#3d6b66', marginTop: 4 }}>{fu.follow_up_date ? format(new Date(fu.follow_up_date), 'EEEE, MMMM d, yyyy') : '—'} · {fu.notes || 'General check-up'}</div>
              </div>
