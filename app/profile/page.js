...
export default function ProfilePage() {
  ...
  if (!profile) return <div style={{ textAlign: 'center', padding: '4rem', color: '#9ca3af' }}>{t('loading')}</div>;
    ...
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', gap: 4, background: '#f5f5f5', borderRadius: 12, padding: 4, width: 'fit-content', marginBottom: 20 }}>
        {tabs.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', fontFamily: 'Poppins', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', background: activeTab === tab ? 'white' : 'transparent', color: activeTab === tab ? '#f08000' : '#4a5568', boxShadow: activeTab === tab ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s', textTransform: 'capitalize' }}>
            {tab === 'followups' ? t('follow_ups') : tab === 'prescriptions' ? t('prescriptions_label') : t('profile')}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'profile' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 20 }}>
          <div className="card">
            <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', marginBottom: 16 }}>{t('personal_info')}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                { label: t('full_name'), key: 'full_name', type: 'text' },
                { label: t('phone'), key: 'phone', type: 'tel' },
                { label: t('age'), key: 'age', type: 'number' },
                { label: t('address'), key: 'address', type: 'text' },
                { label: t('emergency_contact'), key: 'emergency_contact', type: 'tel' },
              ].map(field => (
                <div key={field.key}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#4a5568', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('gender')}</label>
                {editing ? (
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#4a5568', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('blood_group')}</label>
                {editing ? (
          ...
          <div className="card">
            <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', marginBottom: 16 }}>{t('medical_info')}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                { label: t('allergies'), key: 'allergies' },
                { label: t('chronic_conditions'), key: 'chronic_conditions' },
                ...(profile.role === 'doctor' ? [{ label: t('speciality'), key: 'speciality' }] : []),
              ].map(field => (
                <div key={field.key}>
                  <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#4a5568', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>{field.label}</label>
                  {editing ? (
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
            <button onClick={() => setAddingRx(!addingRx)} className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Pill size={16} /> {t('add_prescription')}
            </button>
          </div>

          {addingRx && (
            <div className="card" style={{ marginBottom: 16 }}>
              <h4 style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', marginBottom: 14 }}>{t('new_prescription')}</h4>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
                {[
                  { label: t('medicine_name'), key: 'medicine', placeholder: 'e.g. Paracetamol 500mg' },
                  { label: t('dosage'), key: 'dosage', placeholder: 'e.g. 1 tablet twice daily' },
                  { label: t('duration'), key: 'duration', placeholder: 'e.g. 5 days' },
                  { label: t('notes'), key: 'notes', placeholder: 'Additional instructions' },
                ].map(f => (
                  <div key={f.key}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                <button onClick={() => setAddingRx(false)} className="btn-outline">{t('cancel')}</button>
                <button onClick={addPrescription} className="btn-primary">{t('save_prescription')}</button>
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {prescriptions.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
                ...
                <p style={{ color: '#4a5568' }}>{t('no_prescriptions')}</p>
              </div>
              ...
              <div key={rx.id} className="prescription-card">
                ...
                <div style={{ padding: '1rem 1.5rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
                    {[
                      { label: t('dosage'), value: rx.dosage || '—' },
                      { label: t('duration'), value: rx.duration || '—' },
                      { label: t('prescribed_by'), value: rx.doctor?.full_name ? `Dr. ${rx.doctor.full_name}` : '—' },
                    ].map(d => (
                      <div key={d.label}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {followUps.length === 0 ? (
            ...
            <div key={fu.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e' }}>{t('followup_with')} {fu.doctor?.full_name}</div>
                <div style={{ fontSize: '0.8rem', color: '#4a5568', marginTop: 4 }}>{fu.follow_up_date ? format(new Date(fu.follow_up_date), 'EEEE, MMMM d, yyyy') : '—'} · {fu.notes || t('general_checkup')}</div>
              </div>
