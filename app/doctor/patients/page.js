    ...
    <div className="animate-fade-in" style={{ display: 'flex', gap: 20, height: 'calc(100vh - 80px)' }}>
      ...
      <div style={{ width: activePatient ? 280 : '100%', flexShrink: 0, display: 'flex', flexDirection: 'column', transition: 'width 0.3s' }}>
        <div style={{ marginBottom: 16 }}>
          <h1 style={{ fontFamily: 'Poppins', fontWeight: 800, fontSize: '1.4rem', color: '#1a1a2e' }}>{t('my_patients')}</h1>
          <p style={{ color: '#4a5568', fontSize: '0.8rem', marginTop: 2 }}>Select a patient to view records & prescriptions</p>
        </div>
        <div style={{ position: 'relative', marginBottom: 12 }}>
          <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af' }} />
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search patients..."
            style={{ width: '100%', padding: '9px 10px 9px 32px', borderRadius: 10, border: '1px solid #f0e8d8', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' }} />
        </div>
          ...
          {loading ? (
            ...
            return (
              <button key={patient.id} onClick={() => openPatient(patient)}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: isActive ? '#fff8f0' : 'white', borderRadius: 12, border: `2px solid ${isActive ? '#f08000' : '#f0e8d8'}`, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
                <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg,#10b981,#047857)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontFamily: 'Poppins', flexShrink: 0, fontSize: '0.9rem' }}>
                  {(patient.full_name || patient.name)?.[0]?.toUpperCase()}
                </div>
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <div style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', fontSize: '0.85rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{patient.full_name || patient.name}</div>
                ...
                {isActive && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#f08000', flexShrink: 0 }} />}
              </button>
            );
          })}
        </div>
      ...
      {activePatient && (
        <div style={{ flex: 1, background: 'white', borderRadius: 16, border: '1px solid #f0e8d8', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          {/* Header */}
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #f0e8d8', display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#10b981,#047857)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontFamily: 'Poppins', fontSize: '1.1rem' }}>
              {(activePatient.full_name || activePatient.name)?.[0]?.toUpperCase()}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', fontSize: '1rem' }}>{activePatient.full_name || activePatient.name}</div>
              <div style={{ fontSize: '0.72rem', color: '#4a5568' }}>
                ...
                {activePatient.allergies && <span style={{ color: '#ef4444', fontWeight: 600 }}> · ⚠️ {activePatient.allergies}</span>}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <Link href="/chat" style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', borderRadius: 9, background: '#ecfdf5', color: '#10b981', fontWeight: 600, fontSize: '0.75rem', textDecoration: 'none' }}>
                <MessageCircle size={12} /> Chat
              </Link>
              <button onClick={() => setActivePatient(null)} style={{ padding: '7px', borderRadius: 9, border: '1px solid #f0e8d8', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid #f0e8d8', background: '#fafafa' }}>
            {[
              ...
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                style={{ padding: '10px 18px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: activeTab === tab.key ? 700 : 400, color: activeTab === tab.key ? '#f08000' : '#4a5568', fontSize: '0.82rem', borderBottom: `2px solid ${activeTab === tab.key ? '#f08000' : 'transparent'}`, transition: 'all 0.15s' }}>
                {tab.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px' }}>
            {detailLoading ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem' }}>
                <Loader size={24} style={{ animation: 'spin 1s linear infinite', color: '#f08000' }} />
              </div>
            ) : activeTab === 'records' ? (
              <div>
                {records.length === 0 ? (
                  ...
                  <div key={r.id} style={{ padding: '12px 14px', background: '#fafafa', borderRadius: 12, border: '1px solid #f0e8d8', marginBottom: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg,#f08000,#c66200)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ color: 'white', fontFamily: 'Poppins', fontWeight: 800, fontSize: '0.9rem', lineHeight: 1 }}>{new Date(r.appointment_date).getDate()}</span>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#1a1a2e' }}>{r.time_slot}</div>
                        <div style={{ fontSize: '0.75rem', color: '#4a5568', marginTop: 1 }}>{r.reason || 'General Consultation'}</div>
                        {r.notes && <div style={{ fontSize: '0.72rem', color: '#6b7280', marginTop: 3, fontStyle: 'italic' }}>{r.notes}</div>}
                      </div>
                      <span style={{ fontSize: '0.68rem', padding: '3px 8px', borderRadius: 20, background: r.status === 'completed' ? '#ecfdf5' : r.status === 'confirmed' ? '#eff6ff' : '#fef2f2', color: r.status === 'completed' ? '#065f46' : r.status === 'confirmed' ? '#1d4ed8' : '#991b1b', fontWeight: 700 }}>
                        {r.status}
                      </span>
                    </div>
                  </div>
              ...
              <div>
                ...
                <div style={{ background: '#fff8f0', border: '1px solid #ffd9a0', borderRadius: 14, padding: '14px', marginBottom: 16 }}>
                  <div style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', fontSize: '0.88rem', marginBottom: 12 }}>
                    ➕ New Prescription for {activePatient.full_name || activePatient.name}
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                    <div>
                      <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#4a5568', marginBottom: 3, display: 'block' }}>Medicine *</label>
                    ...
                    <div>
                      <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#4a5568', marginBottom: 3, display: 'block' }}>Dosage</label>
                    ...
                    <div>
                      <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#4a5568', marginBottom: 3, display: 'block' }}>Frequency</label>
                    ...
                    <div>
                      <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#4a5568', marginBottom: 3, display: 'block' }}>Duration</label>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#4a5568', marginBottom: 3, display: 'block' }}>Notes</label>
                ...
                {prescriptions.length === 0 ? (
                  ...
                  <div key={rx.id} style={{ padding: '12px 14px', background: 'white', borderRadius: 12, border: '1px solid #f0e8d8', marginBottom: 8, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <div style={{ width: 36, height: 36, borderRadius: 9, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Pill size={15} color="white" />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#1a1a2e' }}>{rx.medicine}</div>
                      <div style={{ fontSize: '0.75rem', color: '#4a5568', marginTop: 2 }}>{rx.dosage}{rx.duration ? ` · ${rx.duration}` : ''}</div>
