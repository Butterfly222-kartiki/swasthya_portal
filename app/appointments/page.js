...
export default function AppointmentsPage() {
  ...
  return (
    <div className="animate-fade-in">
      <div className="apts-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontFamily: 'Poppins', fontWeight: 800, fontSize: 'clamp(1.2rem,3vw,1.5rem)', color: '#1a1a2e' }}>{t('appointments')}</h1>
          <p style={{ color: '#4a5568', fontSize: '0.875rem', marginTop: 4 }}>Book and manage your consultations</p>
        </div>
        <div style={{ display: 'flex', gap: 8, background: '#f5f5f5', borderRadius: 12, padding: 4, flexShrink: 0 }}>
          {['book', 'list'].map(v => (
            <button key={v} onClick={() => setView(v)} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', fontFamily: 'Poppins', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', background: view === v ? 'white' : 'transparent', color: view === v ? '#f08000' : '#4a5568', boxShadow: view === v ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>
              {v === 'book' ? t('book_appointment') : t('my_appointments')}
            </button>
          ))}
        </div>
      </div>
      ...
      {view === 'book' ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            {[t('select_doctor'), t('pick_date'), t('choose_time'), t('confirm')].map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: '0.75rem', fontWeight: step === i + 1 ? 600 : 400, color: step === i + 1 ? '#f08000' : '#4a5568' }} className="step-label">{s}</span>
                {i < 3 && <div style={{ width: 20, height: 2, background: step > i + 1 ? '#10b981' : '#e5e7eb', borderRadius: 2, transition: 'all 0.3s' }} />}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="card" style={{ gridColumn: '1 / -1' }}>
              <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, marginBottom: 16, color: '#1a1a2e' }}>{t('select_doctor')}</h3>
                ...
                {doctors.map(doc => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#10b981', fontWeight: 600, marginTop: 2 }}>⭐ 4.8 · {t('available_today')}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {step === 4 && (
            <div className="card">
              <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', marginBottom: 16 }}>{t('confirm_booking')}</h3>
              <div style={{ background: '#fafafa', borderRadius: 12, border: '1px solid #f0e8d8', padding: 16, marginBottom: 16 }}>
                {[
                  { label: t('doctor'), value: `Dr. ${selectedDoctor?.full_name}` },
                  { label: t('speciality'), value: selectedDoctor?.speciality || 'General Physician' },
                  { label: t('select_date'), value: selectedDate && format(selectedDate, 'EEEE, MMMM d, yyyy') },
                  { label: t('select_time'), value: selectedSlot },
                ].map(({ label, value }) => (
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '0.85rem', marginBottom: 6, color: '#1a1a2e' }}>{t('reason_visit')}</label>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setStep(3)} className="btn-outline" style={{ flex: 1 }}>{t('back')}</button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {appointments.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
              ...
              <p style={{ color: '#4a5568' }}>{t('no_apts_yet')}</p>
      ...
      <style>{`
        @media (max-width: 640px) {
          ...
          .apts-header { flex-direction: column; align-items: flex-start; }
        }
      `}</style>
    </div>
  );
