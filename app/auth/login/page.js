    ...
    <div className="rangoli-bg" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ width: 60, height: 60, borderRadius: 18, background: 'linear-gradient(135deg,#f08000,#c66200)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', boxShadow: '0 8px 24px rgba(240,128,0,0.35)' }}>
            <HeartPulse size={30} color="white" />
          </div>
          <h1 style={{ fontFamily: 'Poppins', fontWeight: 800, fontSize: '1.6rem', color: '#1a1a2e' }}>Swasthya Portal</h1>
          <p style={{ color: '#4a5568', fontSize: '0.875rem', marginTop: 4 }}>Sign in to your account</p>
        </div>
        ...
        <div className="card" style={{ padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowLang(!showLang)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, border: '1px solid #f0e8d8', background: '#fff8f0', cursor: 'pointer', fontSize: '0.8rem', color: '#f08000', fontWeight: 600 }}>
                <Globe size={13} />
                {supportedLanguages.find(l => l.code === language)?.flag} {supportedLanguages.find(l => l.code === language)?.label}
              </button>
              {showLang && (
                <div style={{ position: 'absolute', top: '110%', left: 0, background: 'white', border: '1px solid #f0e8d8', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 100, minWidth: 150 }}>
                  {supportedLanguages.map(lang => (
                    <button key={lang.code} onClick={() => { changeLanguage(lang.code); setShowLang(false); }}
                      style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 14px', background: language === lang.code ? '#fff8f0' : 'white', border: 'none', cursor: 'pointer', fontSize: '0.85rem', color: language === lang.code ? '#f08000' : '#1a1a2e', fontWeight: language === lang.code ? 600 : 400 }}>
            ...
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, border: '1px solid #e0e7ff', background: '#eef2ff', cursor: 'pointer', fontSize: '0.8rem', color: '#4f46e5', fontWeight: 600 }}
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  style={{ paddingLeft: 38, paddingRight: 44, borderColor: activeField === 'email' ? '#4f46e5' : undefined }}
                />
                <button type="button" onClick={() => activeField === 'email' ? stopVoice() : startVoice('email')}
                  style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 28, height: 28, borderRadius: '50%', border: 'none', background: activeField === 'email' ? '#ef4444' : '#f5f5f5', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  title="Voice input for email"
                >
                  {activeField === 'email' ? <MicOff size={13} color="white" /> : <Mic size={13} color="#4a5568" />}
                </button>
              </div>
            </div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', display: 'flex', gap: 4 }}>
                  <button type="button" onClick={() => setShowPass(!showPass)} style={{ width: 28, height: 28, borderRadius: '50%', border: 'none', background: '#f5f5f5', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {showPass ? <EyeOff size={13} color="#4a5568" /> : <Eye size={13} color="#4a5568" />}
                  </button>
                  <button type="button" onClick={() => activeField === 'password' ? stopVoice() : startVoice('password')}
                    style={{ width: 28, height: 28, borderRadius: '50%', border: 'none', background: activeField === 'password' ? '#ef4444' : '#f5f5f5', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    title="Voice input for password"
                  >
                    {activeField === 'password' ? <MicOff size={13} color="white" /> : <Mic size={13} color="#4a5568" />}
                  </button>
                </div>
              </div>
            </div>
            ...
            {listening && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: '#fef2f2', borderRadius: 8, border: '1px solid #fecaca' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#ef4444', animation: 'pulse 1s infinite' }} />
                <span style={{ color: '#ef4444', fontSize: '0.8rem', fontWeight: 600 }}>Listening… speak now</span>
              </div>
            )}
          ...
          <p style={{ textAlign: 'center', color: '#4a5568', fontSize: '0.875rem' }}>
            ...
            <Link href="/auth/register" style={{ color: '#f08000', fontWeight: 700, textDecoration: 'none' }}>{t('register')}</Link>
          </p>
        </div>
