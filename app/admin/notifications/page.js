  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: 'Poppins', fontWeight: 800, fontSize: '1.5rem', color: '#1a1a2e' }}>Send Notifications</h1>
        <p style={{ color: '#4a5568', fontSize: '0.875rem', marginTop: 4 }}>Broadcast notifications to users via Chrome push</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div style={{ background: 'white', borderRadius: 16, padding: '1.5rem', border: '1px solid #f0e8d8' }}>
          <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', marginBottom: 16 }}>Compose Notification</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#1a1a2e', marginBottom: 6 }}>Send To</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {[['all','All Users'],['patients','Patients'],['doctors','Doctors']].map(([val,label]) => (
                  <button key={val} onClick={() => setTarget(val)}
                    style={{ flex: 1, padding: '8px', borderRadius: 8, border: `2px solid ${target === val ? '#f08000' : '#f0e8d8'}`, background: target === val ? '#fff8f0' : 'white', color: target === val ? '#f08000' : '#4a5568', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', transition: 'all 0.2s' }}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#1a1a2e', marginBottom: 6 }}>Title</label>
              <input className="input-field" value={title} onChange={e => setTitle(e.target.value)} placeholder="Notification title" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: '#1a1a2e', marginBottom: 6 }}>Message</label>
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: '1.5rem', border: '1px solid #f0e8d8', maxHeight: 500, overflow: 'auto' }}>
          <h3 style={{ fontFamily: 'Poppins', fontWeight: 700, color: '#1a1a2e', marginBottom: 14 }}>Recent Notifications</h3>
          {sent.length === 0 ? (
            ...
            <div key={i} style={{ padding: '10px 0', borderBottom: '1px solid #f5f5f5' }}>
              ...
              <div style={{ fontSize: '0.78rem', color: '#4a5568', marginTop: 2 }}>{n.body}</div>
