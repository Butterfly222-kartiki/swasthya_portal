...
export default function ConsultationNotes({ roomId, isDoctor, messages }) {
  ...
  const severityColor = {
    mild: '#10b981', moderate: '#f59e0b', severe: '#ef4444'
  };
  ...
  return (
    <div style={{
      background: 'white', border: '1px solid #f0e8d8', borderRadius: 16,
      ...
      <div
        ...
        style={{
          padding: '10px 14px', background: 'linear-gradient(135deg,#1a1a2e,#312e81)',
          display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer',
        }}
      >
        <Brain size={15} color="#a5b4fc" />
        <span style={{ color: 'white', fontFamily: 'Poppins', fontWeight: 700, fontSize: '0.82rem', flex: 1 }}>
          AI Consultation Notes
        </span>
        {loading && <Loader size={13} color="#a5b4fc" style={{ animation: 'spin 1s linear infinite' }} />}
        {isDoctor && !loading && (
          <button
          ...
          {!notes ? (
            <div style={{ textAlign: 'center', padding: '1rem', color: '#9ca3af' }}>
              {isDoctor ? (
                <>
                  ...
                  {messages?.length >= 2 && (
                    <button onClick={generateNotes} style={{ marginTop: 8, padding: '6px 14px', borderRadius: 8, background: '#4f46e5', color: 'white', border: 'none', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600 }}>
                      Generate Now
                    </button>
                  )}
                </>
            ...
            <>
              ...
              {notes.chiefComplaint && (
                <div style={{ background: '#fff8f0', border: '1px solid #ffd9a0', borderRadius: 10, padding: '8px 12px' }}>
                  <div style={{ fontSize: '0.65rem', fontWeight: 700, color: '#f08000', marginBottom: 3, textTransform: 'uppercase', letterSpacing: 0.5 }}>Chief Complaint</div>
                  <div style={{ fontWeight: 600, color: '#1a1a2e', fontSize: '0.82rem' }}>{notes.chiefComplaint}</div>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {notes.duration && notes.duration !== 'Not specified' && (
                  <div style={{ flex: 1, background: '#eef2ff', border: '1px solid #c7d2fe', borderRadius: 8, padding: '6px 10px', textAlign: 'center' }}>
                    <div style={{ fontSize: '0.62rem', fontWeight: 700, color: '#4f46e5', textTransform: 'uppercase' }}>Duration</div>
                    <div style={{ fontWeight: 700, color: '#4f46e5', fontSize: '0.8rem' }}>{notes.duration}</div>
                  </div>
                )}
              </div>
              ...
              {notes.symptoms?.length > 0 && (
                <div>
                  <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#4a5568', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Symptoms</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {notes.symptoms.map((s, i) => (
                      <span key={i} style={{ padding: '2px 8px', borderRadius: 20, background: '#f5f5f5', fontSize: '0.72rem', color: '#1a1a2e', fontWeight: 500 }}>
                        • {s}
                      </span>
                    ))}
                  </div>
              ...
              {notes.doctorInstructions?.length > 0 && (
                <div>
                  <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#4a5568', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Instructions</div>
                  {notes.doctorInstructions.map((inst, i) => (
                    <div key={i} style={{ fontSize: '0.78rem', color: '#1a1a2e', padding: '3px 0', display: 'flex', alignItems: 'flex-start', gap: 5 }}>
                      <span style={{ color: '#10b981', flexShrink: 0 }}>✓</span> {inst}
                    </div>
                  ))}
                </div>
              )}
              ...
              {notes.redFlags?.length > 0 && (
                <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '8px 12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
                    ...
                    <div style={{ fontSize: '0.65rem', fontWeight: 700, color: '#ef4444', textTransform: 'uppercase' }}>Red Flags</div>
                  </div>
              ...
              {/* Summary */}
              {notes.summary && (
                <div style={{ background: '#f8f9fa', borderRadius: 8, padding: '8px 12px', borderLeft: '3px solid #f08000' }}>
                  <div style={{ fontSize: '0.65rem', fontWeight: 700, color: '#f08000', marginBottom: 3, textTransform: 'uppercase' }}>Summary</div>
                  <div style={{ fontSize: '0.78rem', color: '#4a5568', lineHeight: 1.5 }}>{notes.summary}</div>
                </div>
              )}
