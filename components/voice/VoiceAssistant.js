...
export default function VoiceAssistant() {
  ...
  if (!open) {
    return (
      <button
        ...
        style={{
          position: 'fixed', bottom: 'calc(env(safe-area-inset-bottom) + 80px)', right: 24, zIndex: 999,
      ...
      >
        ...
        <style>{`
          @keyframes voicePulse {
            ...
            50%{box-shadow:0 6px 32px rgba(13,148,136,0.75),0 0 0 10px rgba(13,148,136,0.1)}
          }
        `}</style>
      </button>
    );
  ...
  return (
    <div style={{
      position: 'fixed', bottom: 'calc(env(safe-area-inset-bottom) + 76px)', right: 20, zIndex: 1000,
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: 'white', fontFamily: 'DM Sans', fontWeight: 700, fontSize: '0.875rem', lineHeight: 1 }}>Swasthya AI</div>
      ...
      {!minimized && (
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0 }}>
            {messages.map((msg) => (
              <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                {msg.role === 'user' ? (
                  <div style={{ background: 'linear-gradient(135deg,#0d9488,#0f766e)', color: 'white', borderRadius: '14px 14px 4px 14px', padding: '8px 12px', maxWidth: '80%', fontSize: '0.82rem', lineHeight: 1.5 }}>
                    {msg.text}
                  </div>
                ) : msg.role === 'system' ? (
                  ...
                  <div style={{ maxWidth: '90%' }}>
                    <div style={{ background: '#f8f9fa', border: '1px solid #ccfbf1', color: '#0f2d2a', borderRadius: '14px 14px 14px 4px', padding: '8px 12px', fontSize: '0.82rem', lineHeight: 1.5, marginBottom: msg.liveData ? 8 : 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        <div style={{ width: 16, height: 16, borderRadius: '50%', background: 'linear-gradient(135deg,#0d9488,#0f766e)', flexShrink: 0 }} />
                        <span style={{ fontSize: '0.65rem', color: '#0d9488', fontWeight: 700 }}>SWASTHYA AI</span>
                      </div>
                      {msg.text}
                    </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
                        {msg.liveData.doctors.slice(0,3).map(doc => (
                          <button key={doc.id}
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 10, border: '1px solid #ccfbf1', background: 'white', cursor: 'pointer', textAlign: 'left', width: '100%', transition: 'all 0.15s' }}
                            onMouseEnter={e => e.currentTarget.style.borderColor = '#0d9488'}
                            onMouseLeave={e => e.currentTarget.style.borderColor = '#ccfbf1'}
                          >
                            <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#0f766e,#0d9488)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '0.8rem', flexShrink: 0 }}>
                              {doc.full_name?.[0]}
                            </div>
                            <div>
                              <div style={{ fontWeight: 700, fontSize: '0.78rem', color: '#0f2d2a' }}>Dr. {doc.full_name}</div>
                              <div style={{ fontSize: '0.68rem', color: '#3d6b66' }}>{doc.speciality}</div>
                            </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 5, marginTop: 6 }}>
                        {msg.liveData.slots.slice(0,9).map(slot => (
                          <button key={slot} onClick={() => processText(`I want the ${slot} slot`)}
                            style={{ padding: '5px 4px', borderRadius: 7, border: '1px solid #ccfbf1', background: 'white', cursor: 'pointer', fontSize: '0.68rem', fontWeight: 600, color: '#0f2d2a', transition: 'all 0.15s' }}
                            onMouseEnter={e => { e.currentTarget.style.background = '#0d9488'; e.currentTarget.style.color = 'white'; }}
                            onMouseLeave={e => { e.currentTarget.style.background = 'white'; e.currentTarget.style.color = '#0f2d2a'; }}
                          >
                            {slot}
                          </button>
                        ))}
            ...
            {processing && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f8f9fa', borderRadius: 12, padding: '8px 12px', width: 'fit-content' }}>
                <Loader size={12} color="#0d9488" style={{ animation: 'spin 1s linear infinite' }} />
                <span style={{ fontSize: '0.75rem', color: '#3d6b66' }}>Thinking...</span>
              </div>
            )}
            ...
            <div style={{ padding: '0 12px 8px', display: 'flex', flexWrap: 'wrap', gap: 5 }}>
              {quickCommands.map((cmd, i) => (
                <button key={i} onClick={() => processText(cmd.text)}
                  style={{ padding: '4px 10px', borderRadius: 20, border: '1px solid #ccfbf1', background: 'white', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600, color: '#3d6b66', transition: 'all 0.15s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = '#f0fdfa'; e.currentTarget.style.color = '#0d9488'; e.currentTarget.style.borderColor = '#0d9488'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'white'; e.currentTarget.style.color = '#3d6b66'; e.currentTarget.style.borderColor = '#ccfbf1'; }}
                >
                  {cmd.label}
                </button>
              ))}
          ...
          {/* Input area */}
          <div style={{ padding: '10px 12px', borderTop: '1px solid #ccfbf1', flexShrink: 0 }}>
            {permissionDenied && (
              <div style={{ fontSize: '0.7rem', color: '#059669', marginBottom: 6, textAlign: 'center' }}>
                Mic blocked. Use text input below. ↓
              </div>
            )}

            {recording && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8, padding: '6px', background: '#ecfdf5', borderRadius: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#059669', animation: 'pulse 1s infinite' }} />
                <span style={{ fontSize: '0.75rem', color: '#059669', fontWeight: 600 }}>{t('listening')}</span>
                <button onClick={stopRecording} style={{ padding: '3px 10px', borderRadius: 20, border: 'none', background: '#059669', color: 'white', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600 }}>Stop</button>
              </div>
            )}
            ...
            <form onSubmit={handleTextSubmit} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                ...
                style={{ flex: 1, padding: '8px 12px', borderRadius: 20, border: '1px solid #ccfbf1', fontSize: '0.82rem', outline: 'none', fontFamily: 'Noto Sans, sans-serif' }}
              ...
              <button
                ...
                style={{
                  ...
                  background: recording ? '#059669' : 'linear-gradient(135deg,#0d9488,#0f766e)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                  boxShadow: recording ? '0 0 0 4px rgba(239,68,68,0.2)' : '0 4px 12px rgba(13,148,136,0.35)',
